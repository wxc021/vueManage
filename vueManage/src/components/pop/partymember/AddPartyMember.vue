...
<div class="AddPartyMember">
	<Modal v-model="modal1" title="新增党员" width="950" :styles="{top: '20px'}" :mask-closable=false class-name="max-height vertical-center-modal" :loading="loading" :transfer=true @on-ok="handleSubmit('PartyMemberForm')" @on-cancel="cancel">
		<Form ref="PartyMemberForm" :model="PartyMemberForm" :label-width="150" :rules="ruleValidate">
			<Row>
				<Col span="11">
					<FormItem label="头像：" prop="">
						<div class="" style="height:100%;width:100%; position: realtive;">
					...
					<FormItem label="出生日期：" prop="birthday">
						<DatePicker :transfer=true :options="options" :style="{width:'100%'}" v-model="PartyMemberForm.birthday" type="date" format="yyyy-MM-dd" placeholder="选择日期"></DatePicker>
					</FormItem>
					...
					<FormItem label="参加工作日期：" prop="">
						<DatePicker :transfer=true :options="options" v-model="PartyMemberForm.begin_work_date" type="date" format="yyyy-MM-dd" placeholder="选择日期" style="width:100%"></DatePicker>
					</FormItem>
					...
					<FormItem label="其他党团开始日期：" prop="">
						<DatePicker :transfer=true :options="options" v-model="PartyMemberForm.other_party_time" type="date" format="yyyy-MM-dd" filterable placeholder="选择日期" style="width:100%"></DatePicker>
					</FormItem>
				...
				<Col span="12" offset='1'>
					...
					<FormItem label="毕业日期：" prop="education_end_time">
						<DatePicker :transfer=true v-model="PartyMemberForm.education_end_time" type="date" format="yyyy-MM-dd" placeholder="选择日期" style="width:100%"></DatePicker>
					</FormItem>
					...
					<FormItem label="学位授予日期：" prop="">
						<DatePicker :transfer=true v-model="PartyMemberForm.degree_time" type="date" format="yyyy-MM-dd" placeholder="选择日期" style="width:100%"></DatePicker>
					</FormItem>
					<FormItem label="工作岗位开始日期：" prop="work_start_time">
						<DatePicker :transfer=true v-model="PartyMemberForm.work_start_time" type="date" format="yyyy-MM-dd" placeholder="选择日期" style="width:100%"></DatePicker>
					</FormItem>
					...
					<FormItem label="入党日期：" prop="party_date">
						<DatePicker :transfer=true v-model="PartyMemberForm.party_date" type="date" format="yyyy-MM-dd" placeholder="选择日期" style="width:100%"></DatePicker>
					</FormItem>
					<!--这个还没有-->
					<FormItem label="入党类型：" prop="join_source">
						...
						<Cascader :render-format="renders" :transfer=true :data="join_source_list" v-model="PartyMemberForm.join_source"></Cascader>
						
					</FormItem>
					<!--这个还没有 end-->
					<FormItem label="转正日期：" prop="join_party_date">
						<DatePicker :transfer=true v-model="PartyMemberForm.join_party_date" type="date" format="yyyy-MM-dd" placeholder="选择日期" style="width:100%"></DatePicker>
					</FormItem>
					<FormItem label="转正情况：" prop="to_normal_type">
						<!-- <Select v-model="PartyMemberForm.to_normal_type" clearable filterable placeholder="请选择" :transfer=true>
						<Option v-for='item in to_normal_type_list' :key="item.id" :value="item.id" :disabled="item.choosed==0">{{item.name}}</Option>
					</Select> -->
						<Cascader :render-format="renders" :transfer=true :data="to_normal_type_list" v-model="PartyMemberForm.to_normal_type"></Cascader>
					
					</FormItem>
					<FormItem :label-width='180' label="进入当前党支部日期：" prop="join_dept_time">
						<DatePicker :transfer=true v-model="PartyMemberForm.join_dept_time" type="date" format="yyyy-MM-dd" placeholder="选择日期" style="width:100%"></DatePicker>
					</FormItem>
					<FormItem :label-width='180' label="进入当前党支部类型：" prop="join_dept_type">
...
export default {
	...
	data() {
		...
		return {
			...
			ruleValidate: {
				...
				join_dept_type: [{
					required: true,
					message: '进入当前党支部类型不能为空',
					trigger: 'change'
				}],

				// to_normal_type: [
				//   {required: true, message: '转正情况不能为空', trigger: 'change'}
				// ],
				// join_party_date: [
				//   {required: true,type:'date', message: '转正时间不能为空', trigger: 'change'}
				// ],
			},
			filters: {
				dateFormat(value) {
	...
	watch: {
		'PartyMemberForm.join_party_date' (newval, oldval) {
			console.log(newval);
		...
		'PartyMemberForm.to_normal_type' (newval, oldval) {
			if (newval != "") {
				this.addRule()
			} else {
				this.resetRule()
	...
	methods: {
		...
		resetRule() {
			delete this.ruleValidate.to_normal_type;
			delete this.ruleValidate.join_party_date;
			//this.$refs['PartyMemberForm'].resetFields();
		},
		addRule() {
			this.ruleValidate['to_normal_type'] = [{
				required: true,
				message: '转正情况不能为空',
				trigger: 'change',
				type:'array'
			}];
			this.ruleValidate['join_party_date'] = [{
				required: true,
				type: 'date',
				message: '转正时间不能为空',
				trigger: 'change'
		...
		handleSubmit(name) {
			let that = this;
			console.log(this.PartyMemberForm)
