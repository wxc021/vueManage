    ...
    <div class="CombinationQuery">
        <Modal
            ...
            <Form style="position: relative" ref="CombinationQueryForm" :model="CombinationQueryForm"
                ...
                <Row>
                    <div class="party"><label class="">当前查询单位</label>:<span>{{partyName}}</span>
                    </div>
                    <p class="bold">条件设置</p>
                    <Col span="5">
        ...
        methods: {
            ...
            changeOption(value){
                ...
                this.axios({
                        ...
                        .then(()=>{
                            if(that.singleDataJson.fieldname=='native'){
                                let area = require('@/lib/area.js');
                                that.data=area.default;
                                return ;
                            }else{
                                that.axios({
                                            method:'get',
                            ...
                            .then((res)=>{
                                ...
                                if(needData.code=='200'){
                                    if (needData.data.length==0){
                                        ...
                                        this.$store.commit('setCombMemberData',{postDataArr:needData.data});
                                        this.$store.commit('setIsComQuery',{isComQuery:true});
