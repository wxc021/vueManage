			...
			'BatchUpdateForm.type' (newval,oldval){
				...
				switch (newval) {
					case 'work':
						...
						this.BatchUpdateForm.data = Object.assign({},this.workform);
						break;
					case 'education':
						this.getList('DT10','education_type_list');
						this.getList('GB6864','degree_list');
						this.getList('A08001A','education_list');
						this.BatchUpdateForm.data = Object.assign({},this.eduform);
						break;
				}
			},
		},
				...
				this.$refs[name].validate((valid) => {
						if (valid) {
							...
							if(that.BatchUpdateForm.type=='work'){
								postData.data = Object.assign({},that.workform)
							}else if(that.BatchUpdateForm.type=='education'){
								postData.data = Object.assign({},that.eduform)
							}
							...
							if(that.BatchUpdateForm.type=='work'){
                                let work_start_time = new Date(postData.data.work_start_time).getTime();
                                let work_end_time = new Date(postData.data.work_end_time).getTime();
								...
								if(postData.data.work_type==""||postData.data.work_start_time==""){
                                    ...
                                    that.$Message.error('工作岗位/开始时间为不能大于结束时间')
                                    setTimeout(() => {
                                        that.loading = false;
                                        that.$nextTick(() => {
                                            that.loading = true;
							...
							.then(res=>{
								...
								if (needData.code == 200) {
									...
									this.modal1=false;
									this.$router.push({path:'/dangyuan/'+this.dept_id+'?type='+this.type2});
								} else {
									that.$Message.error(needData.msg);
									setTimeout(() => {
										that.loading = false;
