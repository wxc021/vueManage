<template>
    <div class="tables  tables-auto">
        <div class="title text-center">
           <h3 class="text-center wxc-table-style-title">预备党员转正情况</h3>
        ...
        <div class="table-outer"  style="margin-top:-10px;">
            <div class="start-box">
              <table align=center cellspacing=0 cellpadding=0>
                  <tr>
                      <td width="192" rowspan=3 style="height: 100px;">项目</td>
                      <td width="112" rowspan=3>预备期满的党员</td>
                      <td style="height:40px;" colspan=6>已经上级党组织审批</td>
                      <td width="112" rowspan=3>尚未讨论或审批</td>
                      <td width="112" rowspan=3>预备期未满被取消预备党员资格</td>
                  </tr>
                  <tr>
                     <td class="bl0" rowspan=2>合计</td>
                     <td class="bb0 br0" height="15"></td>
                     <td colspan=2 style="height:15px;"></td>
                     <td rowspan=2>延长预备期</td>
                     <td style="border-right:1px solid #222;" rowspan=2>取消预备党员资格</td>
                  </tr>
                  <tr>
                     <td class="bl0">转为正式党员</td> 
                     <td>延长预备期转正</td>
                     <td style="border-right: 1px solid #222;">推迟讨论转正</td>
                  </tr>
                  <tr class="comm-style">
                     <td height="30">甲</td>
                     <td>A</td>
                  ...
                  <tr v-for="(item,index) in tablesData1">
                    ...
                    <td width="112" v-for="(inner, inx) in item.lists">
                        <input type="text" :disabled="inner.newDis" :class="{'yellow2': inner.isYellow}" v-model="inner.value" :style="{'color': inner.disabled ? '#111' : inner.class, 'backgroundColor': (inx == 0 || inx == 1) ? '#88e1fe' : (index == 0 && inx == 4) ? '#ffce9a' : '' }">
                    </td>
	...
	export default {
		...
		data() {
			return {
                msg: 'this is table',
                tablesData: [
                    {type: 1,title: '', label: '人数', calc: '',lists:[], bgcolor: "#222",},
                ],
    ...
    methods: {
            ...
            init() {
                ...
                this.$watch('origin', function(newV, oldV) {
                    // A1 = H1 + B1
                    let tp1 = this.origin[0].lists[1].value == '' ? 0 : this.origin[0].lists[1].value;
                    let tp2 = this.origin[0].lists[7].value == '' ? 0 : this.origin[0].lists[7].value; 
                    this.origin[0].lists[0].value = parseInt(tp1) + parseInt(tp2);
                    // B1 = C1 + F1 + G1
                    let tp3 = this.origin[0].lists[2].value == '' ? 0 : this.origin[0].lists[2].value; 
                    let tp4 = this.origin[0].lists[5].value == '' ? 0 : this.origin[0].lists[5].value; 
                    let tp5 = this.origin[0].lists[6].value == '' ? 0 : this.origin[0].lists[6].value; 
                    this.origin[0].lists[1].value = parseInt(tp3) + parseInt(tp4) + parseInt(tp5);
                    for (let i = 0; i < this.origin.length; i++) {
