<template>
    <div class="tables tables-auto">
        <div class="title text-center">
           <h3 class="text-center wxc-table-style-title">民主评议党员情况</h3>
        ...
        <div class="table-outer" style="margin-top: -10px;">
            <table align=center cellspacing=0 cellpadding=0 width="100%">
                <tr>
                    <td class="justify" style="padding:0 50px;"  width="315" height="170" colspan=2 rowspan=2>项目</td>
                    <td class="justify bb0 br0" style="height:22px;" colspan=2></td>
                    <td class="justify bl0" style="height:22px;"></td>
                    <td class="justify" rowspan=2>建制村</td>
                    <td class="justify" rowspan=2>社区（居委会）</td>
                    <td class="justify" rowspan=2>机关</td>
                    <td class="justify" rowspan=2>事业单位</td>
                    <td class="justify" rowspan=2>公有制经济控制企业</td>
                    <td class="justify" rowspan=2>非公有制经济控制企业</td>
                    <td class="justify" rowspan=2>社会组织</td>
                    <td class="justify" rowspan=2>其它</td>
                </tr>
                <tr>
                    <td class="justify bl0" colspan=2>总数</td>
                    <td class="justify" style="border-right:1px solid #222;">上半年开展本年结束评议</td>
                </tr>
                <tr class="comm-style">
                    <td colspan=2>甲</td>
                    <td colspan=2 wclass="comm-reset"idth="94">A</td>
                    <td>B</td>
                    <td>C</td>
                    <td>D</td>
                    <td>E</td>
                ...
                <tr width="94" v-for="(item, index) in tablesData">
                    ...
                    <td colspan=2  class="justify"  v-if="item.type == 8">{{item.label}}</td>
                    ...
                    <td style="border-left: 1px solid #222;" rowspan=4 class="justify" width='20' v-if="item.type == 7">{{item.title}}</td>
                    <td class="justify bl0" v-show="item.type == 7">{{item.label}}</td>
                    
                    <td class="justify bl0" v-show="item.type == 5">{{item.label}}</td>

                    <td style="border-left:1px solid #222;" rowspan=9 class="justify" width='20' v-if="item.type == 6">{{item.title}}</td>
                    ...
                    <td width="80" v-for="(inner, inx) in item.lists">
                        <input  v-if="inner.disabled"  type="text" :disabled="inner.disabled" :class="{'yellow2': inner.isYellow}" v-model="inner.value" :style="{'color': inner.disabled ? '#111' : inner.class, 'backgroundColor': (inx == 0 || index == 6) ? '#88e1fe' : inx == 8 || inx == 9 || index == 11 ? '#ffce9a' : '' }">
                        <input v-if="!inner.disabled" type="text" :disabled="inner.newDis" :class="{'yellow2': inner.isYellow}" v-model="inner.value" :style="{'color': inner.disabled ? '#111' : inner.class, 'backgroundColor': (inx == 0 || index == 6) ? '#88e1fe' : inx == 8 || inx == 9 || index == 11 ? '#ffce9a' : '' }">
                    </td>
        ...
        <div class="tips-outer start-box">
                ...
                <div class="tips box-flex wxc-table-style-footer">
                    <p>本年民主评议党员中，发现党员违反党的纪律线索，通报纪检机关查实开除党籍的<input type="text" :disabled="footerVal[0].disabled" :style="{color: footerVal[0].class,'backgroundColor':'#ffce9a'}" v-model="footerVal[0].value" >名,名，给予其他党的纪律处分的<input type="text" :disabled="footerVal[1].disabled" :style="{color: footerVal[1].class,'backgroundColor':'#ffce9a'}" v-model="footerVal[1].value">名。</p>
                </div>
            </div>
	...
	export default {
		...
		data() {
			return {
                msg: 'this is table',
                tablesData: [  
                    {type: 1,title: '', label: '开展评议的支部',  disabled: false, calc: '', bgcolor: "#222", lists: []},
                    {type: 2,title: '', label: '结束评议的支部', calc: '', disabled: false, bgcolor: "#222", lists: []},
                    {type: 1,title: '', label: '参加评议的党员',  disabled: false, calc: '', bgcolor: "#222", lists: []},
                    {type: 2,title: '', label: '结束评议的党员', calc: '', disabled: false, bgcolor: "#222", lists: []},
                    {type: 8,title: '', label: '表彰党员',  disabled: false, calc: '', bgcolor: "#222", lists: []},
                    {type: 8,title: '', label: '被评定为“不合格党员”',  disabled: false, calc: '', bgcolor: "#222", lists: []},

                    {type: 4, title: '组织处置情况', label: '合计', calc: '',  disabled: false, bgcolor: "#222", lists: []},
                    {type: 5, title: '', label: '限期改正', calc: '', disabled: false, bgcolor: "#222", lists: []},
                    {type: 5, title: '', label: '劝退', calc: '', disabled: false, bgcolor: "#222", lists: []},
                    {type: 5, title: '', label: '券而不退除名', calc: '', disabled: false, bgcolor: "#222", lists: []},
                    {type: 5, title: '', label: '自行脱党除名', calc: '', disabled: false, bgcolor: "#222", lists: []},
                    {type: 8,title: '', label: '限期改正期满仍无转变予以劝退以及劝而不退除名',  disabled: false, calc: '', bgcolor: "#222", lists: []},
                ],
                orignData: {},
                lists: [], 
                colors: [],
            ...
            init() {
                ...
                this.$watch('origin', function(newV, oldV) {
                    ...
                    for (let i = 1; i < 10; i++) {
                        ...
                        this.origin[6].lists[i].value = temp;
                    };
