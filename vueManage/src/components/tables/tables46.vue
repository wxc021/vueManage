<template>
    <div class="tables  tables-auto">
        <div class="title text-center">
           <h3 class="text-center wxc-table-style-title">基层党组织领导班子公推直选情况</h3>
        ...
        <div class="table-outer"  style="margin-top:-10px;">
            ...
            <div class="start-box">
              <table align=center cellspacing=0 cellpadding=0>
                  <tr>
                      <td style="height:80px;" rowspan="2" width="240">项目</td>
                      <td colspan=5>开展公推直选的基层党组织</td>
                      <td colspan="2">书记、副书记选举方式</td>
                  </tr>
                  <tr>
                      <td width="220" colspan="2" class="bl0">总数</td>
                      <td width="160">党委</td>
                      <td width="160">总支部</td>
                      <td width="160">支部</td>
                      <td width="160">差额选举</td>
                      <td width="160">等额选举</td>
                  </tr>
                  <tr class="comm-style">
                     <td width="240" height="36" >甲</td>
                     <td  width="160" colspan=2>A</td>
                     <td width="160">B</td>
                     <td width="160">C</td>
                     <td width="160">D</td>
                     <td width="160">E</td>
                  ...
                  <tr v-for="(item,index) in tablesData1">
                    ...
                    <td :class="{'justify': true}" v-if="item.type == 1" width="160">{{item.label}}</td>
            ...
            <!--<div class="tips-outer start-box">-->
                ...
                <!--<div class="tips box-flex">-->
                    <!--<p>-->
                        <!--1.受到纪律处理的党组织-->
                        <!--<input type="text" :disabled="footerVal[0].disabled"  :style="{color: footerVal[0].class,'backgroundColor':'#ffce9a'}" class="bg-class3" v-model="footerVal[0].value" >个，其中改组-->
                        <!--<input type="text" :disabled="footerVal[1].disabled"  :style="{color: footerVal[1].class,'backgroundColor':'#ffce9a'}" class="bg-class3" v-model="footerVal[1].value" >个、解散-->
                        <!--<input type="text" :disabled="footerVal[2].disabled"  :style="{color: footerVal[2].class,'backgroundColor':'#ffce9a'}" class="bg-class3" v-model="footerVal[2].value">个。-->
                    <!--</p>-->
                <!--</div>-->
            <!--</div>-->
        </div>
	...
	export default {
		...
		data() {
			return {
                msg: 'this is table',
                tablesData: [
                    {type: 1,title: '', label: '乡镇', calc: 'let temp=0;for(let i=1;i<6;i++){let tp=this.origin[0].lists[i].value==""?0:this.origin[0].lists[i].value;if(i<4){temp+=parseInt(tp)}};this.origin[0].lists[0].value=temp', disabled: false, bgcolor: "#222", lists:[]},
                    {type: 1,title: '', label: '建制村', calc: 'let temp=0;for(let i=1;i<6;i++){let tp=this.origin[1].lists[i].value==""?0:this.origin[1].lists[i].value;if(i<4){temp+=parseInt(tp)}};this.origin[1].lists[0].value=temp', disabled: false, bgcolor: "#222", lists:[]},
                    {type: 1,title: '', label: '社区（居委会）', calc: 'let temp=0;for(let i=1;i<6;i++){let tp=this.origin[2].lists[i].value==""?0:this.origin[2].lists[i].value;if(i<4){temp+=parseInt(tp)}};this.origin[2].lists[0].value=temp', disabled: false, bgcolor: "#222", lists:[]},
                    {type: 1,title: '', label: '机关', calc: 'let temp=0;for(let i=1;i<6;i++){let tp=this.origin[3].lists[i].value==""?0:this.origin[3].lists[i].value;if(i<4){temp+=parseInt(tp)}};this.origin[3].lists[0].value=temp', disabled: false, bgcolor: "#222", lists:[]},
                    {type: 1,title: '', label: '事业单位', calc: 'let temp=0;for(let i=1;i<6;i++){let tp=this.origin[4].lists[i].value==""?0:this.origin[4].lists[i].value;if(i<4){temp+=parseInt(tp)}};this.origin[4].lists[0].value=temp', disabled: false, bgcolor: "#222", lists:[]},
                    {type: 1,title: '', label: '公有经济控制企业', calc: 'let temp=0;for(let i=1;i<6;i++){let tp=this.origin[5].lists[i].value==""?0:this.origin[5].lists[i].value;if(i<4){temp+=parseInt(tp)}};this.origin[5].lists[0].value=temp', disabled: false, bgcolor: "#222", lists:[]},
                    {type: 1,title: '', label: '非公有经济控制企业', calc: 'let temp=0;for(let i=1;i<6;i++){let tp=this.origin[6].lists[i].value==""?0:this.origin[6].lists[i].value;if(i<4){temp+=parseInt(tp)}};this.origin[6].lists[0].value=temp', disabled: false, bgcolor: "#222", lists:[]},
                    {type: 1,title: '', label: '社会组织', calc: 'let temp=0;for(let i=1;i<6;i++){let tp=this.origin[7].lists[i].value==""?0:this.origin[7].lists[i].value;if(i<4){temp+=parseInt(tp)}};this.origin[7].lists[0].value=temp', disabled: false, bgcolor: "#222", lists:[]},
                    {type: 1,title: '', label: '其他', calc: 'let temp=0;for(let i=1;i<6;i++){let tp=this.origin[8].lists[i].value==""?0:this.origin[8].lists[i].value;if(i<4){temp+=parseInt(tp)}};this.origin[8].lists[0].value=temp', disabled: false, bgcolor: "#222", lists:[]},
                ],
                orignData: {},
                lists: [], 
                colors: [],
