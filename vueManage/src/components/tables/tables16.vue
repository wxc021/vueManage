<template>
    <div class="tables tables-auto">
        <div class="title text-center">
           <h3 class="text-center wxc-table-style-title">党员出党情况(二)</h3>
        ...
        <div class="table-outer" style="margin-top: -10px;">
            <table align=center cellspacing=0 cellpadding=0 width="100%">
                <tr>
                    <td class="justify" width="315" colspan=3 height="50" style="padding: 0 80px;">项目</td>
                    <td class="justify" colspan=2>总数</td>
                    <td class="justify">开除党籍</td>
                    <td class="justify">退党除名</td>
                    <td class="justify">劝退</td>
                    <td class="justify">劝而不退除名</td>
                    <td class="justify">自行脱党除名</td>
                    <td class="justify">取消预备党员资格</td>
                    <td class="justify">不予承认</td>
                </tr>
                <tr class="comm-style">
                   <td colspan=3>甲</td>
                ...
                <tr width="95" v-for="(item, index) in tablesData">
                    ...
                    <!-- <td colspan=3  class="justify bb0"  v-if="item.type == 1">{{item.label}}</td> -->
                    <td width='20'  rowspan=7  class="justify" v-if="item.type == 2">{{item.title}}</td>
                    <td colspan=2 class="justify" v-if="item.type == 2">{{item.label}}</td>    

                    <!-- <td  class="justify bb0" width='20' v-if="item.type == 2"></td>
                    <td style="border-top:1px solid #222;"  colspan=2 class="justify" v-if="item.type == 2">{{item.label}}</td> -->

                    <td  class="justify bl0 bb0 br0" width='20' v-if="item.type == 3"></td>
                    <td  class="justify" v-if="item.type == 3">{{item.label}}</td>

                    <td  class="justify bl0" width='20' v-if="item.type == 33"></td>
                    <td  class="justify" v-if="item.type == 33">{{item.label}}</td>

                    <td rowspan=4 class="justify" width='20' v-if="item.type == 4">{{item.title}}</td>
                    <td class="justify bl0" colspan=2 v-show="item.type == 4">{{item.label}}</td>
                    
                    <td class="justify bl0" colspan=2 v-show="item.type == 5">{{item.label}}</td>

                    <td style="border-left:2px solid #222;" rowspan=6 class="justify" width='20' v-if="item.type == 6">{{item.title}}</td>
                    <td class="justify bl0" colspan=2 v-show="item.type == 6">{{item.label}}</td>

                    <td class="comm-reset" width=30>{{index+1}}</td>

                    <td width="80" v-for="(inner, inx) in item.lists">
                        <input  v-if="inner.disabled"  type="text" :disabled="inner.disabled" :class="{'yellow2': inner.isYellow}" v-model="inner.value" :style="{'color': inner.disabled ? '#111' : inner.class, 'backgroundColor': (index == 0 || inx == 0) ? '#88e1fe' : ((inx == 6 || inx == 3 || inx == 4) && index > 7 && index < 14) || (inx == 7 && index == 14) ? '#ffce9a' : '' }">
                        <input v-if="!inner.disabled" type="text" :disabled="inner.newDis" :class="{'yellow2': inner.isYellow}" v-model="inner.value" :style="{'color': inner.disabled ? '#111' : inner.class, 'backgroundColor': (index == 0 || inx == 0) ? '#88e1fe' : ((inx == 6 || inx == 3 || inx == 4) && index > 7 && index < 14) || (inx == 7 && index == 14) ? '#ffce9a' : '' }">
                    </td>
    ...
    export default {
        ...
        data() {
            return {
                msg: 'this is table',
                tablesData: [  
                    {type: 1,title: '', label: '总计',  disabled: false, calc: 'let temp = 0;for (let i = 1; i < 8; i++) {let tp = this.origin[0].lists[i].value == "" ? 0 : this.origin[0].lists[i].value; temp += parseInt(tp)};this.origin[0].lists[0].value = temp', bgcolor: "#222", lists: []},

                    {type: 2,title: '违纪行为', label: '违反政治纪律', calc: '', disabled: false, bgcolor: "#222", lists: []},

                    {type: 3,title: '', label: '违反组织纪律', calc: '', disabled: false, bgcolor: "#222", lists: []},
                    {type: 33,title: '', label: '违反干部选拔任用规定', calc: '', disabled: false, bgcolor: "#222", lists: []},
                    {type: 5,title: '', label: '违反廉洁纪律', calc: '', disabled: false, bgcolor: "#222", lists: []},
                    {type: 5,title: '', label: '违反群众纪律', calc: '', disabled: false, bgcolor: "#222", lists: []},
                    {type: 5,title: '', label: '违反工作纪律', calc: '', disabled: false, bgcolor: "#222", lists: []},
                    {type: 5,title: '', label: '违反生活纪律', calc: '', disabled: false, bgcolor: "#222", lists: []},


                    {type: 6, title: '具体表现', label: '理想信念缺失', calc: '',  disabled: false, bgcolor: "#222", lists: []},
                    {type: 5, title: '', label: '政治立场动摇', calc: '', disabled: false, bgcolor: "#222", lists: []},
                    {type: 5, title: '', label: '宗旨观念淡薄', calc: '', disabled: false, bgcolor: "#222", lists: []},
                    {type: 5, title: '', label: '工作消极懈怠', calc: '', disabled: false, bgcolor: "#222", lists: []},

                    {type: 5, title: '', label: '组织纪律散漫', calc: '', disabled: false, bgcolor: "#222", lists: []},
                    {type: 5, title: '', label: '道德行为不端', calc: '', disabled: false, bgcolor: "#222", lists: []},
        ...
        methods: {
            ...
            init() {
                ...
                for (let i = 0; i < this.tablesData.length; i++) {
                    if (i > 0 && i < 14) {
                        that.tablesData[i].lists.forEach((item, index) => {
                            if ((i > 7 && i < 14 && index == 1) || (i > 0 && i < 14 && (index == 2 || index == 7)) || (i > 0 && i < 8 && (index == 3 || index == 4) || (i > 0 && i < 14 && i != 12 && index == 5))) {
                ...
                this.$watch('origin', function(newV, oldV) {
                    ...
                    for (let i = 1; i < 15; i++) {
                        ...
                        if (i > 0 && i < 8) {
                            ...
                            let tp1 = this.origin[i].lists[1].value == "" ? 0 : this.origin[i].lists[1].value;
                            let tp2 = this.origin[i].lists[6].value == "" ? 0 : this.origin[i].lists[6].value;

                            temp += ( parseInt(tp1)+parseInt(tp2) );
                            
                        } else if(i > 7 && i < 14) {
                            let tp1 = this.origin[i].lists[3].value == "" ? 0 : this.origin[i].lists[3].value;
                            let tp2 = this.origin[i].lists[4].value == "" ? 0 : this.origin[i].lists[4].value;
                            let tp3 = this.origin[i].lists[6].value == "" ? 0 : this.origin[i].lists[6].value;


                            temp += ( parseInt(tp1)+parseInt(tp2)+parseInt(tp3) );
                        } else {
                            for (let j = 1; j < 8; j++) {
