<template>
    <div class="tables tables-auto">
        <div class="title text-center">
           <h3 class="text-center wxc-table-style-title">党员基本情况</h3>
        ...
        <div class="table-outer" style="margin-top: -10px;">
            <table align=center cellspacing=0 cellpadding=0>
                  <tr>
                      <td width="350" height="90" colspan=3 style="height:90px;">项目</td>
                      <td width="160" colspan=2>城市街道</td>
                      <td width="160">城市社区（居委会）</td>
                      <td width="160">乡镇</td>
                      <td width="160">乡镇社区（居委会）</td>
                      <td width="160">建制村</td>
                  </tr>
                  <tr class="comm-style">
                  ...
                  <tr v-for="(item,index) in tablesData">
                    ...
                    <td :class="{'justify': true}" v-if="item.type == 11" colspan=3 class="text-left">{{item.label}}</td>
                    ...
                    <td :class="{'justify': true}" v-if="item.type == 33" colspan="2" style="border-left: 0px;">{{item.label}}</td>
                    ...
                    <td width="90" :class="{'justify': true}"  v-if="item.type == 4" rowspan="3">{{item.title}}</td>
                    <td :class="{'justify': true}"  v-if="item.type == 4" colspan="2">{{item.label}}</td>

                    <td :class="{'justify': true}" style="border-bottom-width: 2px;" v-if="item.type == 44" rowspan="2">{{item.title}}</td>
                    <td :class="{'justify': true}"  v-if="item.type == 44" colspan="2">{{item.label}}</td>

                    <td width="90" :class="{'justify': true}" style="border-top:1px solid #222;"  v-if="item.type == 7" rowspan="4">{{item.title}}</td>
                    <td width="30" class="bb0 br0" v-if="item.type == 7"></td>
                    <td :class="{'justify': true}"  v-if="item.type == 7">{{item.label}}</td>

                    <!-- <td :class="{'justify': true}"  v-if="item.type == 8" rowspan="4">{{item.title}}</td> -->
                    <td width="30" style="border-bottom: 1px solid #222;" class="bl0" v-if="item.type == 8"></td>
                    <td :class="{'justify': true}"  v-if="item.type == 8">{{item.label}}</td>

                    
                    <td :class="{'justify': true}" v-if="item.type == 5" colspan="2" class="bl0">{{item.label}}</td>
                    
                    <td width="30" class="comm-reset">{{index+1}}</td>
                    <td width="160" v-for="(inner, inx) in item.lists">
                        <input  v-if="inner.disabled"  type="text" :disabled="inner.disabled" :class="{'yellow2': inner.isYellow}" v-model="inner.value" :style="{'color': inner.disabled ? '#111' : inner.class, 'backgroundColor': (index == 0) ? '#88e1fe' : (index == 12) ? '#ffce9a' : '' }">
                        <input v-if="!inner.disabled" type="text" :disabled="inner.newDis" :class="{'yellow2': inner.isYellow}" v-model="inner.value" :style="{'color': inner.disabled ? '#111' : inner.class, 'backgroundColor': (index == 0) ? '#88e1fe' : (index == 12) ? '#ffce9a' : '' }">
                    </td>
                  </tr>
              </table>
            <div class="tips-outer start-box">
                <p style="margin-top:6px;">补充资料：</p>
                <div class="tips box-flex wxc-table-style-footer">
                    <p>本表A1栏中包括建立工委的城市街道<input class="bg-class3" type="text" :disabled="footerVal[0].disabled" :style="{color: footerVal[0].class}" v-model="footerVal[0].value">个。</p>
                </div>
            </div>
        </div>
	...
	export default {
		...
		data() {
			return {
                msg: 'this is table',
                tablesData: [
                    {type: 11,title: '', label: '单位数', calc: 'for (var i = 0; i < 5; i++) { let temp = 0; for (let j = 1; j < 11; j++) { if (j != 5 && j != 6 && j != 8) {let ii = this.origin[j].lists[i].value == "" ? 0 : this.origin[j].lists[i].value;  temp += parseInt(ii)};} this.origin[0].lists[i].value = temp;}', disabled: false, bgcolor: "#222", lists:[]},
                    ...
                    {type: 5,title: '', label: '没有党员的', calc: '', disabled: false, bgcolor: "#222", lists:[]},
                     {type: 44,title: '补充情况', label: '建立党员服务机构的', calc: '', disabled: false, bgcolor: "#222", lists:[]},
                     {type: 5,title: '', label: '建立党员志愿者队伍的', calc: '', disabled: false, bgcolor: "#222", lists:[]},
                ],
                orignData: {},
                lists: [], 
                colors: [],
