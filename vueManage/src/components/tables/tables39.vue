<template>
    <div class="tables  tables-auto">
        <div class="title text-center">
           <h3 class="text-center wxc-table-style-title">国务院和地方政府工作部门法人单位建立党组织情况</h3>
        ...
        <div class="table-outer"  style="margin-top:-10px;">
            ...
            <div class="start-box">
              <table align=center cellspacing=0 cellpadding=0>
                  <tr>
                      <td width="230"  style="height:80px;">项目</td>
                      <td width="140" colspan=2>总数</td>
                      <td width="184" style="padding: 0 20px;">国务院部（委、办、局）</td>
                      <td width="184" style="padding: 0 20px;">省（区、市）政府工作部门 </td>
                      <td width="184" style="padding: 0 20px;">市（地、州、盟）政府工作部门 </td>
                      <td width="184" style="padding: 0 20px;"> 县（市、区、旗）政府工作部门 </td>
                  </tr>
                  <tr class="comm-style">
                     <td style="height:50px;">甲</td>
                     <td colspan=2 width="234">A</td>
                     <td width="184">B</td>
                     <td width="184">C</td>
                     <td width="184">D</td>
                     <td width="184">E</td>
                  ...
                  <tr v-for="(item,index) in tablesData1" style="height:80px;">
                    <td :class="{'justify': true}" v-if="item.type == 1" class="text-left">{{item.label}}</td>
                    <td :class="{'justify': true}" v-if="item.type == 2" class="text-left">{{item.label}}</td>

                    <td width="50" class="comm-reset">{{index+1}}</td>
                    <td width="184" v-for="(inner, inx) in item.lists" v-if="inx<5">
                        <input type="text" :disabled="inner.newDis" :class="{'yellow2': inner.isYellow}" v-model="inner.value" :style="{'color': inner.disabled ? '#111' : inner.class, 'backgroundColor': (inx == 0 || index==0) ? '#88e1fe' :  '#ffce9a' }">
            ...
            <div class="tips-outer start-box">
                ...
                <div class="tips box-flex wxc-table-style-footer">
                    <p>军队和武警部队团及团以上单位建立的党委<input type="text" :disabled="footerVal[0].disabled" :style="{color: footerVal[0].class}" v-model="footerVal[0].value" >个。</p>
                </div>
            </div>
        </div>
	...
	export default {
		...
		data() {
			return {
                msg: 'this is table',
                tablesData: [
                    {type: 1,title: '', label: '部门数', calc: 'for(let i=0;i<5;i++){let tp1=this.origin[1].lists[i].value==""?0:this.origin[1].lists[i].value;let tp2=this.origin[2].lists[i].value==""?0:this.origin[2].lists[i].value;let tp3=this.origin[3].lists[i].value==""?0:this.origin[3].lists[i].value;let tp4=this.origin[4].lists[i].value==""?0:this.origin[4].lists[i].value;let tp5=this.origin[5].lists[i].value==""?0:this.origin[5].lists[i].value;this.origin[0].lists[i].value=parseInt(tp1)+parseInt(tp2)+parseInt(tp3)+parseInt(tp4)+parseInt(tp5)}',
                        lists:[], bgcolor: "#222",},



                    {type: 2,title: '', label: '建立党组的', calc: 'let temp = 0;for (let i = 1; i < 5; i++) {let tp = this.origin[1].lists[i].value == "" ? 0 : this.origin[1].lists[i].value; temp += parseInt(tp)};this.origin[1].lists[0].value = temp', disabled: false, bgcolor: "#222", lists:[]},
                    {type: 2,title: '', label: '建立党委的', calc: 'let temp = 0;for (let i = 1; i < 5; i++) {let tp = this.origin[2].lists[i].value == "" ? 0 : this.origin[2].lists[i].value; temp += parseInt(tp)};this.origin[2].lists[0].value = temp', disabled: false, bgcolor: "#222", lists:[]},
                    {type: 2,title: '', label: '建立总支部的', calc: 'let temp = 0;for (let i = 1; i < 5; i++) {let tp = this.origin[3].lists[i].value == "" ? 0 : this.origin[3].lists[i].value; temp += parseInt(tp)};this.origin[3].lists[0].value = temp', disabled: false, bgcolor: "#222", lists:[]},
                    {type: 2,title: '', label: '建立支部的', calc: 'let temp = 0;for (let i = 1; i < 5; i++) {let tp = this.origin[4].lists[i].value == "" ? 0 : this.origin[4].lists[i].value; temp += parseInt(tp)};this.origin[4].lists[0].value = temp', disabled: false, bgcolor: "#222", lists:[]},
                    {type: 2,title: '', label: '未建立党组织的', calc: 'let temp = 0;for (let i = 1; i < 5; i++) {let tp = this.origin[5].lists[i].value == "" ? 0 : this.origin[5].lists[i].value; temp += parseInt(tp)};this.origin[5].lists[0].value = temp', disabled: false, bgcolor: "#222", lists:[]},
                ],
