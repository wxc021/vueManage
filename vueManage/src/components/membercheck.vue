<template>
    <div class="app-main" :style="{'width':'100%','height':'100%'}">
        <div class="layout  start-box box-orient" :style="{'width':'100%','height':'100%'}" @click.stop="closeDefault">
            <!-- 头部 -->
            <top></top>
            <Row type="flex" class='setHeight'>
                <div class="check-form" @click.stop="closeDefault">
...
<script>
    ...
    export default {
        ...
        methods: {
            closeDefault() {
                if (this.$route.name == 'membercheckinfo') {
                    this.$router.go(-1);
                };
            },
            ajax(url, type, data) {
            ...
            compareF(row) {
                event.stopPropagation();
                if (this.$route.name === 'membercheck') {
                    this.$router.push({
