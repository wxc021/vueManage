    ...
    <div class="g-core-image-corp-container" v-bind:id="'vciu-modal-' + formID" v-show="hasImage">
      ...
      <div class="info-aside">
        <p class="btn-groups" v-if="crop">
          <button type="button" v-on:click="doCrop" class="btn btn-upload" style="background:#e3392c; border-color: #e3392c;">{{cropBtn.ok}}</button>
        ...
        <p class="btn-groups" v-if="resize && !crop">
          <button type="button" v-on:click="doResize" class="btn btn-upload" style="background:#e3392c;border-color: #e3392c;">确定</button>
    ...
    methods: {
      ...
      doCrop(e) {
        ...
        if (this.crop === 'local') {
          ...
          if (this.data.size > 1024*1024) {
            this.data.compress = 30;
          } else {
            this.data.compress = 50;
          }
          return canvasHelper.crop(targetImage, this.data, (code) => {
            let pd = code.split(',')[1];
            pd = window.atob(pd);
            var ia = new Uint8Array(pd.length);
            for (var i = 0; i < pd.length; i++) {
                ia[i] = pd.charCodeAt(i);
            };
            let blob = new Blob([ia], {type:"image/png",filename: this.data.filename});
            upload(code);
            this.__dispatch('imagechanged', blob);
          })
        }
        upload();
      },
          ...
          .done(function(res) {
            if (res) {
              if (res.code == 200) {
                ...
                // data.guid = res.data.guid;
                // data.append('uptoken',res.data.uptoken);
                data.uptoken = res.data.uptoken;
                xhr('POST',self.url, self.headers, data, done, errorUpload, isBinary);               
              };
            };
